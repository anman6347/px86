...
void set_memory8(Emulator *emu, uint32_t address, uint32_t value) {
    emu->memory[address] = value & 0xFF;  // 8bit 値を設定するため
}
...
void update_eflags_sub(Emulator *emu, uint32_t v1, uint32_t v2, uint64_t result) {
    int sign1 = v1 >> 31;
    int sign2 = v2 >> 31;
    int signr = (result >> 31) & 1;  // 33bit 以降の処理

    set_carry(emu, result >> 32);  // result の 33bit 以降が 0 かそうでないか
    set_zero(emu, result == 0);
    set_sign(emu, signr);
    set_overflow(emu, sign1 != sign2 &&
                          sign1 != signr);  // sub のオーバーフロー検出に注意
}
